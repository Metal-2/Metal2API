...
class UsuarioControlador extends Controladores {
    ...
    function validarUsuario(){
        ...
        $validateProviderAuthData = $this->validateProviderAuthData;
        $mensaje = ""; 
    ...
    function todos() {
      ...
      if(is_null($inicioBusqueda) && is_null($busqueda)){
        $usuarios=Usuarios::porRango(0,$cantidad);
        $cantidadTotal=Usuarios::cantidadTotal();
          
        echo Respuestas::exito(['Datos'=>$usuarios,'CantidadTotal'=>$cantidadTotal]);
       
      }else{
         if(empty($busqueda) || is_null($busqueda)){
         	$usuarios=Usuarios::porRango($inicioBusqueda,$cantidad);
         }else{
			$usuarios=Usuarios::porFiltroYRango($busqueda,$inicioBusqueda,$cantidad);
         }
         echo Respuestas::exito($usuarios);
      }
    ...
    function permisosItemMenu(){
        $usuarioID=$this->usuarioID;
        $usuario=Usuarios::dato($usuarioID);
        if($usuario){
            $items=[];
            $menuComponentes=ControlOperaciones::todosComponentes();
            if ($usuario->usuarioTIPO=="ADMIN" || $usuario->usuarioTIPO=="SUPER ADMIN") {
                $items= ControlOperaciones::todosMenu();
            }else{
            ...
            echo Respuestas::exito("item menus",["Componentes"=>$menuComponentes,"Items"=>$items]);
        }else{
